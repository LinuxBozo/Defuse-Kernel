...
#include <../drivers/sensor/optical/capella_cm3663.h>
#include <linux/regulator/max8998.h>
#ifdef CONFIG_REGULATOR_MAX8893
#include <linux/regulator/max8893.h>		//NAGSM_Android_SEL_Kernel_Aakash_20101005
#include <linux/regulator/bh6173.h>            //NAGSM_Android_SEL_Kernel_Aakash_20101018
#endif 
#include <linux/clk.h>
...
#include <mach/gpio.h>
#include <mach/gpio-dempsey-setting.h>
...
#include <mach/pn544_i2c.h>
//gpio-crespo.h
#define NFC_SCL_18V			S5PV210_MP04(6)
#define NFC_SDA_18V			S5PV210_MP04(7)
...
static struct s3c2410_uartcfg smdkv210_uartcfgs[] __initdata = {
	...
	[1] = {
		...
		.ufcon		 = S3C2410_UFCON_FIFOMODE | S5PV210_UFCON_TXTRIG64 | S5PV210_UFCON_RXTRIG8, //S5PV210_UFCON_RXTRIG1 -> S5PV210_UFCON_RXTRIG8 edited by scott // -> RX trigger leve : 8byte.
#else 
		.ufcon		= S5PV210_UFCON_DEFAULT,
#endif
#endif
...
static struct regulator_init_data max8998_ldo9_data = {
        .constraints    = {
                .name           = "PM_MAX8893_VDD",
                .min_uV         = 2800000,
                .max_uV         = 2800000,
...
#ifdef CONFIG_REGULATOR_MAX8893
        { MAX8998_LDO9, &max8998_ldo9_data },		//NAGSM_Android_SEL_Kernel_Aakash_20101005
#endif
...
/* NAGSM_Android_SEL_Kernel_Subhransu_20101004 */
...
void lcd_cfg_gpio_early_suspend(void)
{
	...
	s3c_gpio_setpull(GPIO_DISPLAY_HSYNC, S3C_GPIO_PULL_NONE);
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	s3c_gpio_setpin(GPIO_DISPLAY_HSYNC, 1);
#else
	s3c_gpio_setpin(GPIO_DISPLAY_HSYNC, 0);
#endif
	...
	s3c_gpio_setpull(GPIO_DISPLAY_VSYNC, S3C_GPIO_PULL_NONE);
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	s3c_gpio_setpin(GPIO_DISPLAY_VSYNC, 1);
#else
	s3c_gpio_setpin(GPIO_DISPLAY_VSYNC, 0);
#endif
	...
	s3c_gpio_setpull(GPIO_DISPLAY_DE, S3C_GPIO_PULL_NONE);
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	s3c_gpio_setpin(GPIO_DISPLAY_DE, 1);
#else
	s3c_gpio_setpin(GPIO_DISPLAY_DE, 0);
#endif
	...
	s3c_gpio_setpull(GPIO_DISPLAY_PCLK, S3C_GPIO_PULL_NONE);
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	s3c_gpio_setpin(GPIO_DISPLAY_PCLK, 1);
#else
	s3c_gpio_setpin(GPIO_DISPLAY_PCLK, 0);
#endif
	...
	s3c_gpio_setpull(GPIO_DISPLAY_CLK, S3C_GPIO_PULL_NONE);
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	s3c_gpio_setpin(GPIO_DISPLAY_CLK, 1);
#else
	s3c_gpio_setpin(GPIO_DISPLAY_CLK, 0);
#endif
	/* DISPLAY_SI */
	...
	s3c_gpio_setpull(GPIO_DISPLAY_SI, S3C_GPIO_PULL_NONE);
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	s3c_gpio_setpin(GPIO_DISPLAY_SI, 1);
#else
	s3c_gpio_setpin(GPIO_DISPLAY_SI, 0);
#endif
}
...
static  struct  i2c_gpio_platform_data  i2c14_platdata = {
        .sda_pin                = SUBPM_SDA_28V,
        .scl_pin                = SUBPM_SCL_28V,
        .udelay                 = 2,    /* 250KHz */
        .sda_is_open_drain      = 0,
        .scl_is_open_drain      = 0,
        .scl_is_output_only     = 0,
...
static	struct	i2c_gpio_platform_data	i2c17_platdata = {
	.sda_pin		= GPIO_AP_SDA, //NAGSM_Android_SEL_Kernel_Aakash_20101214
	.scl_pin		= GPIO_AP_SCL, //NAGSM_Android_SEL_Kernel_Aakash_20101214
	.udelay			= 5,	/* 250KHz */		
	.sda_is_open_drain	= 0,
	.scl_is_open_drain	= 0,
	.scl_is_output_only	= 0,
...
static struct sec_jack_port sec_jack_port[] = {
	{
		...
		{ // SEND/END info
#ifdef CONFIG_S5PC110_CELOX_BOARD		
			.eint       = IRQ_EINT1,
#else
			.eint       = IRQ_EINT(30),
#endif
...
{	
	...
	/* Turn CAM_SENSOR_IO_1.6V off */
//	Set_MAX8998_PM_REG(ELDO16, 0);
	udelay(50);

	/* Turn VGA_IO_1.8V off */
#ifdef CONFIG_REGULATOR_MAX8893
	bh6173_ldo_disable_direct(2);//LDO 2
#endif
	udelay(50);
	
	/* Turn CAM_SENSOR_A_2.8V off */
#ifdef CONFIG_REGULATOR_MAX8893
...
static inline int sr130pc10_power_on()
{
	...
	/*CAM_CORE_EN */
	err = gpio_request(CAM_CORE_EN, "GPC1(1)");
	if (err) {
		printk(KERN_ERR "failed to request gpio(CAM_CORE_EN)\n");
		return err;
	}		
...
#ifdef CONFIG_VIDEO_M5MO
/*
 * Guide for Camera Configuration for Seine board
 * ITU CAM CH A: M5MO
*/
static int m5mo_power_on_sr130()
{
	...
	ret = gpio_request(CAM_CORE_EN, "GPC1(1)");
	if (ret) {
		printk(KERN_ERR "failed to request gpio(CAM_CORE_EN)\n");
		return ret;
	}
...
static int m5mo_power_down_sr130()
{
	...
	ret = gpio_request(CAM_CORE_EN, "GPC1(1)");
	if (ret) {
		printk(KERN_ERR "faile to request gpio(CAM_CORE_EN)\n");
		return ret;
	}
...
static struct i2c_board_info i2c_devs1[] __initdata = {
    {
        I2C_BOARD_INFO("s5p_ddc", (0x74>>1)),		//NAGSM_Android_SEL_Kernel_Aakash_20101214
    },
};
...
static struct i2c_board_info i2c_devs5[] __initdata = {
	{
		I2C_BOARD_INFO("kr3dh_accel", 0x19),
	},
#ifdef CONFIG_SENSORS_L3G4200D_GYRO
	{
...
static struct i2c_board_info i2c_devs11[] __initdata = {
	{
		I2C_BOARD_INFO(TAOS_DEVICE_NAME, TMD2711_ADDR),
	},
};
#endif
...
#if defined(CONFIG_OPTICAL_CAPELLA_TRITON)
static struct i2c_board_info i2c_devs11[] __initdata = {
	{
		I2C_BOARD_INFO(CAPELLA_DEVICE_NAME, CM3663_ADDR),
	},
};
#endif
...
static struct modemctl_platform_data mdmctl_data = {
	...
	.gpio_pda_active = GPIO_PDA_ACTIVE, 		/* move to cfg_gpio, hardware variation */
...
#endif
#if 0 //!(defined(CONFIG_S5PC110_KEPLER_BOARD) || defined(CONFIG_S5PC110_T959_BOARD)) || defined(CONFIG_S5PC110_FLEMING_BOARD) // irq_eint(27) is used for fuel_int 
	[1] = {
		.start = IRQ_EINT(27),
		.end = IRQ_EINT(27),
		.flags = IORESOURCE_IRQ,
...
// turn off touchkey	
	...
	#if ! defined (CONFIG_CP_CHIPSET_STE) 
    gpio_set_value(GPIO_CP_ON, 0); //prevent phone reset when AP off
    #else
...
//NAGSM_Android_HQ_Camera_SungkooLee_20101022
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	&s3c_device_i2c16,	//VT_camera bestiq
#endif
//NAGSM_Android_HQ_Camera_SungkooLee_20101022

//MHL v1 //NAGSM_Android_SEL_Kernel_Aakash_20101114
	&s3c_device_i2c17,	
//MHL v1 //NAGSM_Android_SEL_Kernel_Aakash_20101114
...
static void __init smdkc110_machine_init(void)
{
	...
	gpio_request(GPIO_HWREV_MODE0, "GPIO_HWREV_MODE0");	/* XMMC3CLK */
	gpio_export(GPIO_HWREV_MODE0, 0); 
	gpio_request(GPIO_HWREV_MODE1, "GPIO_HWREV_MODE1");	/* XMMC3CLK */
	gpio_export(GPIO_HWREV_MODE1, 0); 
	gpio_request(GPIO_HWREV_MODE2, "GPIO_HWREV_MODE2");	/* XMMC3CLK */
	gpio_export(GPIO_HWREV_MODE2, 0); 
	gpio_request(GPIO_HWREV_MODE3, "GPIO_HWREV_MODE3");	/* XMMC3CLK */
	gpio_export(GPIO_HWREV_MODE3, 0); 
...
#ifdef CONFIG_REGULATOR_MAX8893
//NAGSM_Android_SEL_Kernel_Aakash_20101005
	i2c_register_board_info(14, i2c_devs14, ARRAY_SIZE(i2c_devs14)); /* max8893 */

//NAGSM_Android_SEL_Kernel_Aakash_20101005

//NAGSM_Android_SEL_Kernel_Aakash_20101018
        i2c_register_board_info(15, i2c_devs15, ARRAY_SIZE(i2c_devs15)); /* bh6173 */

//NAGSM_Android_SEL_Kernel_Aakash_20101018
#endif 

	i2c_register_board_info(17, i2c_devs17, ARRAY_SIZE(i2c_devs17)); //MHL v1 //NAGSM_Android_SEL_Kernel_Aakash_20101119
...
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	mdmctl_data.gpio_pda_active = GPIO_PDA_ACTIVE;

#else
	if(HWREV >= 0x1){
		mdmctl_data.gpio_pda_active = GPIO_PDA_ACTIVE_HW_01;
	} else {	
		mdmctl_data.gpio_pda_active = GPIO_PDA_ACTIVE;
	}
#endif 
...
//NAGSM_Android_HQ_Camera_SungkooLee_20101022
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	s3c_gpio_cfgpin( VT_CAM_SCL_18V, 1 );
	s3c_gpio_setpull( VT_CAM_SCL_18V, S3C_GPIO_PULL_UP); 
	s3c_gpio_cfgpin( VT_CAM_SDA_18V, 1 );
	s3c_gpio_setpull( VT_CAM_SDA_18V, S3C_GPIO_PULL_UP); 	
#endif
...
#if 0 // MIDAS[2010.09.04] Remove unnecessary code
...
//NAGSM_Android_HQ_Camera_SungkooLee_20101022
    ...
    else
    {
        ...
        if (!ldo38_onoff_state)
        {
	     ...
	     ldo8_status = 0; //Subhransu20110304
            msleep(1);
//NAGSM_Android_HQ_Camera_SungkooLee_20101022
			ldo3_status &= ~(1 << LDO3_USB);
			if (ldo3_status == 0)
...
int ldo38_control_by_tvout_test()
{
	mutex_lock(&ldo38_mutex);
	//Set_MAX8998_PM_REG(ELDO8, 1 );
	//msleep(1);
	...
	if (ldo3_status == 0)
		Set_MAX8998_PM_REG(ELDO3, 0);
	printk(KERN_ERR "%s : turn OFF LDO3 and LDO8 (cur_stat=%d)\n",__func__,ldo38_onoff_state);
...
*/
	...
	case 1:
		...
		s3c_gpio_cfgpin(GPIO_GPS_TXD, S3C_GPIO_SFN(GPIO_GPS_TXD_AF));
		s3c_gpio_setpull(GPIO_GPS_TXD, S3C_GPIO_PULL_UP);
#endif
		break;
	case 2:
...
static unsigned int wlan_gpio_table[][4] = {	
	...
	{GPIO_WLAN_BT_EN, GPIO_WLAN_nRST_AF, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
...
//For Starting/Stopping Tethering service
	...
	/* mmc_rescan*/
	sdhci_s3c_force_presence_change(&s3c_device_hsmmc3);	//BRCM_CHANGE
	msleep(250);  //wait for being ready for enabling wlan
}
EXPORT_SYMBOL(wlan_setup_power);
#if CONFIG_GPS_CHIPSET_STE_CG2900
